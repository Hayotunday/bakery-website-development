  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-primary/5 bg-white/90 backdrop-blur-md">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        >
          <Image
            alt="Perfect White Bakery Logo"
            src={"/bakery-logo.jpeg"}
            className=""
            priority
            width={50}
            height={50}
        ...
        <nav className="hidden md:flex items-center gap-10">
          {menuItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`text-sm font-semibold text-primary hover:text-accent transition-colors relative 
                after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-accent after:transition-all hover:after:w-full`}
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="hidden md:flex items-center gap-4">
          <Link
            href="/contact"
            className="bg-primary hover:bg-accent text-white text-sm font-bold py-2.5 px-6 
            rounded-full transition-all active:scale-95 shadow-lg shadow-primary/20 uppercase tracking-wider"
          >
            Book Event
          </Link>
        </div>
      ...
      {isOpen && (
        ...
        >
          <div
            className="bg-white h-screen max-h-[calc(100vh-4rem)] overflow-y-auto"
          ...
          >
            ...
            <nav className="flex flex-col px-6 py-8">
              ...
              <Link
                href="/contact"
                onClick={handleNavClick}
                className="block text-center w-full mt-6 bg-primary hover:bg-accent text-white text-sm font-bold py-3 px-6 rounded-full transition-all active:scale-95 uppercase tracking-wider shadow-lg shadow-primary/20"
              >
                Book Event
              </Link>
              ...
              <div className="mt-10 pt-8 border-t border-primary/10 space-y-6">
                <h3 className="text-xs uppercase font-bold text-primary tracking-widest">
                  Get in Touch
                </h3>
                ...
                <a
                  href="mailto:hello@perfectwhite.com"
                  className="flex items-center gap-4 text-primary hover:text-accent transition-colors group"
                >
                  <div className="w-10 h-10 rounded-full bg-secondary group-hover:bg-primary/10 flex items-center justify-center transition-colors">
              ...
              <div className="mt-8 pt-8 border-t border-primary/10">
                ...
                <div className="flex gap-4">
                  <a
                    href="#"
                    className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center hover:bg-accent transition-colors"
                  >
